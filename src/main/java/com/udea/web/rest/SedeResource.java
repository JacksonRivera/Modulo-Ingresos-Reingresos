...
public class SedeResource {
        ...
        Optional<Sede> result = sedeRepository
            ...
            .map(existingSede -> {
                ...
                if (sede.getDireccionSede() != null) {
                    existingSede.setDireccionSede(sede.getDireccionSede());
                }
                if (sede.getRegional() != null) {
                    existingSede.setRegional(sede.getRegional());
                }
    ...
    //Metodo que trae la regional de una sede por su nombre
    @GetMapping("/sedes/nombre/{nombre}")
    public String getSedeNombre(@PathVariable String nombre) {
        Sede Sede = sedeRepository.findByNombreSede(nombre);
        return Sede.getRegional().toString();
    }
